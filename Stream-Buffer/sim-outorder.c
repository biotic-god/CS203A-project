 ...
 */
  ...
  /*********************cs203********************/
static struct cacht_t *pf_streambuffer_i = NULL;
static struct cacht_t *pf_streambuffer_d = NULL;
static int pf_streambuffer_opt;
static int pf_numbers;
static int pf_lat;
  /*********************cs203********************/
	      ...
	      tick_t now)		/* time of access */
{
  unsigned int lat;
    /*********************cs203********************/
  if (pf_streambuffer_d){
      if (cache_prefetchbuffer(pf_streambuffer_d, baddr, bsize, pf_numbers, now)){
          cache_dl1->pf_hits++; 
          cache_dl1->misses--;
          cache_dl1->hits++;
          return 1; 
      }
      else {cache_dl1->pf_misses++;}
  }
  /*********************cs203********************/
  if (cache_dl2)
    {
      ...
      lat = cache_access(cache_dl2, cmd, baddr, NULL, bsize,
			 /* now */now, /* pudata */NULL, /* replace addr */NULL);
	      ...
	      tick_t now)		/* time of access */
{
  unsigned int lat;
  /*********************cs203********************/
    if (pf_streambuffer_i){
            if (cache_prefetchbuffer(pf_streambuffer_i, baddr, bsize, pf_numbers, now)){ 
		cache_il1->pf_hits++; 
                cache_il1->misses--;
                cache_il1->hits++;
                return 1; 
            }
	    else {cache_il1->pf_misses++;}
    }
  /*********************cs203********************/
...
{
  /*********************cs203********************/
  opt_reg_flag(odb, "-pf", "Use a prefetch stream buffer",
         &pf_streambuffer_opt, FALSE, /* print */TRUE, NULL);
  opt_reg_int(odb, "-cache:pflat",
        "victim cache hit latency (in cycles)",
        &pf_lat, /* default */1,
        /* print */TRUE, /* format */NULL);
  opt_reg_int(odb, "-cache:pfblocks",
        "total number of victim cache blocks",
        &pf_numbers, /* default */4,
        /* print */TRUE, /* format */NULL);
  /*********************cs203********************/
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  /*********************cs203********************/
        if (!pf_streambuffer_opt){
            pf_streambuffer_i = NULL;
            pf_streambuffer_d = NULL;
        } else {
            pf_streambuffer_i = cache_create("ipf", 1, bsize, FALSE,
            0, pf_numbers, cache_char2policy('f'), il2_access_fn, pf_lat);
            pf_streambuffer_d = cache_create("dpf", 1, bsize, FALSE,
            0, pf_numbers, cache_char2policy('f'), dl2_access_fn, pf_lat);
        }
  /*********************cs203********************/
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  /*********************cs203********************/
  if (pf_streambuffer_i)
    cache_reg_stats(pf_streambuffer_i, sdb);
  if (pf_streambuffer_d)
    cache_reg_stats(pf_streambuffer_d, sdb);
  /*********************cs203********************/
