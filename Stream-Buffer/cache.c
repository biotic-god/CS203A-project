		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  /*********************cs203********************/
  if (strcmp(name,"ipf") == 0 || strcmp(name,"dpf") == 0 || strcmp(name,"il1") == 0 || strcmp(name,"dl1") == 0){
    sprintf(buf, "%s.pf_misses", name); 
    stat_reg_counter(sdb, buf, "prefetch stream buffer misses", &cp->pf_misses, 0, NULL);
    sprintf(buf, "%s.pf_hits", name);
    stat_reg_counter(sdb, buf, "prefetch stream buffer hits", &cp->pf_hits, 0, NULL);}
  /*********************cs203********************/
}
	    ...
	    FILE *stream)		/* output stream */
{
  ...
  /*********************cs203********************/
unsigned int				
cache_prefetchbuffer(struct cache_t *cp,	
            md_addr_t addr,		
            int nbytes,
            int pf_numbers,		
            tick_t now){	
    ...
    unsigned int lat = 0; unsigned int target_blk = 0; unsigned int position = 0; int i = 1; 
    if (cache_probe(cp, addr)){
        ...
        cp->pf_hits++;
        for (i = 1; i <= target_blk; ++i)
          {cache_access(cp, 0, addr + i * nbytes, NULL, nbytes, now, NULL, NULL);}
        return 1;
    }
    cp->pf_misses++;
    cache_flush(cp, now);
    for (i = 1; i <= pf_numbers; ++i)
      { cache_access(cp, 0, addr + i * nbytes , NULL, nbytes, now , NULL, NULL);}
    return 0;
}
  /*********************cs203********************/
