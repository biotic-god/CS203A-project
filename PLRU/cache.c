	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  /*********************cs203********************/
      cp->sets[i].PLRU_index = (int) pow(2.0 , (double) (cp->assoc - 1)) - 1;
  /*********************cs203********************/     
...
int
PLRU_indexchange ( int blk_entry,   
                int blk_index,
                int PLRU_entry,          
                int PLRU_index_bf ) {
  int PLRU_index_af = PLRU_index_bf;
  int PLRU_index_cmp = (1 << PLRU_entry) - 1;
  int base = 0;
  int n = 0;
  int cmp;
  int bit =0;
  while (n < blk_entry)
  {
    if ((blk_index >> (blk_entry - 1 - n))%2 == 0)
	{bit = 0;}
    else
	{bit = 1;}
    cmp = PLRU_index_cmp & (~(1 << base));
    PLRU_index_af = (PLRU_index_af & cmp) | (bit << base);
    if (bit)
    {base = base*2 + 1;}
    else if (!bit)
    {base = base*2 + 2;}
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case P_LRU:
         { 
	  int PLRU_index_bf = cp->sets[set].PLRU_index;         
          int blk_index = 0;  	  
  	  int base = 0;
  	  int n = 0;
  	  int bit = 0;
	  int blk_entry = log_2(cp->assoc);
  	  int PLRU_entry = cp->assoc - 1;
  	  while (n < blk_entry )
  	  {
    	  if((PLRU_index_bf >> base) % 2 == 0)
    	  {bit = 0;}
	  else
	  {bit = 1;}
    	  if (bit)
          ...
          repl = CACHE_BINDEX(cp, cp->sets[set].blks, blk_index);
          cp->sets[set].PLRU_index = PLRU_indexchange(blk_entry, blk_index, PLRU_entry, PLRU_index_bf);
    	  }
    	  break;
  /*********************cs203********************/
  default:
 ...
 cache_hit: /* slow hit handler */
  ...
  /*********************cs203********************/
  if (blk->way_prev && cp->policy == P_LRU)
  {
      int blk_index = 0;
      int blk_entry = log_2(cp->assoc);
      int PLRU_entry = cp->assoc - 1;
      struct cache_blk_t *walk_blk = blk;
      for (walk_blk = blk; walk_blk != NULL; walk_blk = walk_blk->way_prev)
      {blk_index ++;}
      int PLRU_index_bf = cp->sets[set].PLRU_index;
      cp->sets[set].PLRU_index = PLRU_indexchange ( blk_entry, blk_index, PLRU_entry, PLRU_index_bf );
  }
