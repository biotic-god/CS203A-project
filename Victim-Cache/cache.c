		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  if (!cp->name || !cp->name[0])
    ...
    name = cp->name;
  /*********************cs203********************/
  if (strcmp(name,"vc") == 0 || strcmp(name,"dl1") == 0){
  sprintf(buf, "%s.vc_misses", name); 
  stat_reg_counter(sdb, buf, "victim cache misses", &cp->vc_misses, 0, NULL);
  sprintf(buf, "%s.vc_hits", name);
  stat_reg_counter(sdb, buf, "victim cache hits", &cp->vc_hits, 0, NULL);}
  /*********************cs203********************/
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  /*********************cs203********************/
	cp->rb_address = CACHE_MK_BADDR(cp, repl->tag, set);
  /*********************cs203********************/
