 ...
 */
  ...
  /*********************cs203********************/
static struct cacht_t *cache_vc = NULL;
static int cache_vc_opt; 
static int cache_vc_lat;
static int cache_vc_blocks;
  /*********************cs203********************/
...
{  
    /*********************cs203********************/
  if (cache_vc != NULL && cache_dl1->rb_address != 0){
        if (cache_probe(cache_vc, baddr) != 0)
	{     
	      cache_dl1->misses--; cache_dl1->hits++; cache_dl1->vc_hits++;
              cache_flush_addr(cache_vc, baddr, now); 
              cache_access(cache_vc, 0, cache_dl1->rb_address, NULL, bsize, now, NULL, NULL); 
              return 0;
        } 
	else 
	{
  ...
  if (cache_dl2)
    {
      ...
      lat = cache_access(cache_dl2, cmd, baddr, NULL, bsize,
			 /* now */now, /* pudata */NULL, /* replace addr */repl_addr);
...
{
  /*********************cs203********************/
  opt_reg_flag(odb, "-vc", "victim cache enabled",
         &cache_vc_opt, FALSE, /* print */TRUE, NULL);
  opt_reg_int(odb, "-cache:vclat",
        "victim cache hit latency (in cycles)",
        &cache_vc_lat, /* default */1,
        /* print */TRUE, /* format */NULL);
  opt_reg_int(odb, "-cache:vcblocks",
        "total number of victim cache blocks",
        &cache_vc_blocks, /* default */8,
        /* print */TRUE, /* format */NULL);
  /*********************cs203********************/
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  /*********************cs203********************/
        if (!cache_vc_opt){cache_vc = NULL;} 
	else {cache_vc = cache_create("vc", 1, bsize, FALSE, 0 , cache_vc_blocks, cache_char2policy(c), dl2_access_fn, cache_vc_lat);}
  /*********************cs203********************/
