...
extern int dvfsinterval;

extern double dvfstargetpower;
...
static double sumFSF = 0.0;

static double sumVSF = 0.0;

static double avgFSF = 0.0;

static double avgVSF = 0.0;

static double dvfs_sumpowerperinterval = 0.0;

static double dvfs_avgpowerperinterval = 0.0;

static double dvfs_pertime = 0;

static double dvfs_totaltime = 0;

static double dvfs_finalpower = 0;

static double dvfs_finalenergy = 0;
...
void update_power_stats()
{
  ...
  if(resultbus_access) {
    ...
    resultbus_power_cc3+=turnoff_factor*power.resultbus;
  	...
  	current_total_cycle_power_cc1 = total_cycle_power_cc1 - last_single_total_cycle_power_cc1;
  	current_total_cycle_power_cc2 = total_cycle_power_cc2 - last_single_total_cycle_power_cc2;
  	current_total_cycle_power_cc3 = total_cycle_power_cc3 - last_single_total_cycle_power_cc3;
...
/*------------------------------cs203-----------------------------------*/
	cyclenumber = (int)sim_cycle;
	extern char *ld_prog_fname;
  	double current_sum_cycle_power = current_total_cycle_power_cc1 + current_total_cycle_power_cc2 + current_total_cycle_power_cc3;
	dvfs_sumpowerperinterval  += current_sum_cycle_power;
	dvfs_finalpower += current_sum_cycle_power;
	dvfs_finalenergy += current_sum_cycle_power * dvfs_pertime;
  	dvfs_pertime = 1.0 / (double)(Mhz*FSF);
	dvfs_totaltime += dvfs_pertime;
	
	if(cyclenumber % ((int)dvfsinterval) == 0){
		char filename[100];
		char pfilename2[10];
		char *pfilename1 = &ld_prog_fname[strlen(ld_prog_fname) - 8];
		strncpy(pfilename2, pfilename1, 2);
		pfilename2[2] = '\0'; 
		sprintf(filename,"./result/%s%s%d" , pfilename2, "-result-power=", (int)dvfstargetpower);
		FILE* resultfile;
		if(flag == 1){
			resultfile = fopen(filename,"w");
			if (resultfile != NULL){
				fprintf(resultfile, "%s", "counts FSF VSF dvfs_avgpowerperinterval dvfstargetpower\n");
    				fclose (resultfile);
			}	
			flag = 0;
		}
		dvfs_avgpowerperinterval = dvfs_sumpowerperinterval / dvfsinterval;
		resultfile = fopen(filename,"a");
		if (resultfile != NULL){
			fprintf(resultfile, "%d ", counts);
			fprintf(resultfile, "%lf ", FSF);
			fprintf(resultfile, "%lf ", VSF);
			fprintf(resultfile, "%lf ", dvfs_avgpowerperinterval);
			fprintf(resultfile, "%lf\n", dvfstargetpower);
    			fclose(resultfile);
		}
		if( dvfs_avgpowerperinterval <= dvfstargetpower){
			FSF = MIN(2.0 , FSF * sqrt(dvfstargetpower/dvfs_avgpowerperinterval));
			VSF = MIN(2.0 , VSF * sqrt(dvfstargetpower/dvfs_avgpowerperinterval));
		        calculate_power(&power);
		}
		else{
			FSF = FSF * sqrt(dvfstargetpower/dvfs_avgpowerperinterval);
			VSF = VSF * sqrt(dvfstargetpower/dvfs_avgpowerperinterval);
			calculate_power(&power);
		}

		dvfs_sumpowerperinterval = 0;
		counts++;
    		sumFSF += FSF;
    		avgFSF = sumFSF/counts;
    		sumVSF += VSF;
    		avgVSF = sumVSF/counts;
	}
}
/*------------------------------cs203-----------------------------------*/
...
power_reg_stats(struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  stat_reg_double(sdb, "dvfs_final_power", "final power consumption",&dvfs_finalpower, 0, NULL);  

  stat_reg_double(sdb, "dvfs_final_energy", "final energy consumption", &dvfs_finalenergy, 0, NULL);

  stat_reg_double(sdb, "dvfs_total_time", "total time cost by CPU", &dvfs_totaltime, 0, NULL);

  stat_reg_int(sdb, "dvfs_counts", "total nmber of cycles", &counts, 0, NULL);

  stat_reg_double(sdb, "dvfs_avg_fsf", "average fsf per cycle",&avgFSF, 0, NULL);

  stat_reg_double(sdb, "dvfs_avg_vsf", "average vsf per cycle",&avgVSF, 0, NULL);

  stat_reg_formula(sdb, "dvfs_avgpower", "average power per cycle","dvfs_final_power/sim_cycle", NULL);

  stat_reg_formula(sdb, "dvfs_avgenergy", "average energy per cycle","dvfs_final_energy/sim_cycle", NULL);
